...
QUARANTINE_ROLE_ID  = 1503924411797999659
TRUSTED_ROLE_ID     = 1503796620788109324
TRANSCRIPT_CHAN_ID  = 1503924739176140912
QUARANTINE_CAT_ID   = 1503924954788397228
...
async def create_quarantine_ticket(member: discord.Member, reason: str):
    ...
    try:
        ...
        print(f"[antinuke] Could not create ticket channel: {e}")
...
async def quarantine_member(member: discord.Member, reason: str):
    ...
    try:
        ...
        print(f"[antinuke] Failed to manage roles for {member}: {e}")
        return
...
async def on_member_join(member: discord.Member):
    ...
    try:
        ...
        print(f"[antinuke] Audit log error: {e}")
...
async def on_message(message: discord.Message):
    ...
    if message.mention_everyone and not has_trusted_role(author):
        try:
            ...
            pass
        await quarantine_member(author, reason="Used @everyone / @here without the required role.")
        await bot.process_commands(message)
        return
    ...
    if ping_count > 0:
        ...
        if total >= PING_LIMIT:
            ...
            try:
                ...
                pass
            await quarantine_member(author, reason=f"Ping spam: {total} pings in under {PING_WINDOW} seconds.")
            await bot.process_commands(message)
            return
...
async def cmd_whitelist(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        print(f"[antinuke] /whitelist error: {e}")
...
async def cmd_removequar(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        if qrole is None:
            await interaction.followup.send("❌ Quarantine role not found in this server.", ephemeral=True)
            return
        ...
        print(f"[antinuke] /removequar error: {e}")
        await interaction.followup.send(f"❌ An error occurred: `{e}`", ephemeral=True)
